...
use super::{Boolean, ModuleType, Param, Symbol, Varidiac};

#[derive(Clone, Debug, PartialEq)]
pub struct Pair(pub Ast2, pub Ast2);
...
mod impl_transformer {
    ...
    impl TryFrom<Ast1> for Ast2 {
        type Error = Error;

        fn try_from(value: Ast1) -> Result<Self, Self::Error> {
            match value {
                Ast1::Boolean(b) => Ok(Self::Boolean(b)),
                Ast1::Number(n) => Ok(Self::Number(n)),
                Ast1::String(s) => Ok(Self::String(s)),
                Ast1::Symbol(i) => Ok(Self::Symbol(i)),
                // Ast1::Application(app) => convert_application(app, state),
                Ast1::Label(l) => Ok(Self::Label(l)),
                Ast1::Pair(pair) => todo!(),
                Ast1::TheEmptyList => todo!(),
                Ast1::Syntax(syntax) => todo!(),
            }
        }
    }
    ...
    // fn convert_application(app: Vec<Ast1>) -> Result<Ast2, Error> {
    //     match app.first() {
    //         Some(Ast1::Symbol(i))
