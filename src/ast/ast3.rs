...
mod impl_transformer {
    fn quote(exp: Ast1) -> Ast3 {
        match exp {
            Ast1::Boolean(t) => Ast3::Bool(t),
            Ast1::Number(t) => Ast3::Number(t),
            Ast1::String(t) => Ast3::String(t),
            Ast1::Symbol(t) => Ast3::Ident(t),
            Ast1::Application(t) => Ast3::Application(t.into_iter().map(quote).collect()),
            Ast1::Label(t) => Ast3::Label(t),
            Ast1::FnParam(t) => Ast3::FnParam(t),
            _ => unreachable!(),
        }
    }
    use crate::{
        ast::{ast1::Ast1, map_box, AstTransformFrom, IteratorTransformer},
    ...
    type Error = String;
    impl AstTransformFrom<Ast1> for Ast3 {
        type Error = Error;
        ...
        fn transform(value: Ast1, state: Self::State) -> Result<(Self, Self::State), Self::Error> {
            ...
            match value {
                Ast1::Boolean(t) => Ok((Self::Bool(t), state)),
                Ast1::Module(name, kind) => Ok((Self::Module(name, kind), state)),
                Ast1::Number(t) => Ok((Self::Number(t), state)),
                Ast1::String(t) => Ok((Self::String(t), state)),
                Ast1::Symbol(t) => Ok((Self::Ident(t), state)),
                Ast1::FnParam(t) => Ok((Self::FnParam(t), state)),
                Ast1::Quote(q) => Ok((Self::Quote(map_box(q, quote)), state)),
                Ast1::Application(exprs) => exprs
                    .into_iter()
